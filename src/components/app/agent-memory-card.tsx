...
export function AgentMemoryCard({ snapshots }: AgentMemoryCardProps) {
  return (
    <GlassCard className="p-5" glow="cyan">
      <div className="mb-5 flex items-center justify-between">
        ...
        <Badge variant="cyan">Stateful memory</Badge>
      </div>
      <div className="space-y-3">
        {snapshots.map((snapshot) => (
          ...
          >
            ...
            <p className="mt-2 text-sm text-white/52">{snapshot.detail}</p>
          </div>
        ))}
      </div>
    </GlassCard>
